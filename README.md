# 🚀 Hi, I'm Radu Nicolae

[![Website](https://img.shields.io/badge/Website-radu--nicolae.com-1f6feb?style=flat-square&logo=googlechrome&logoColor=white)](https://radu-nicolae.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-@rnicolae-0a66c2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/rnicolae)

I focus on making distributed computer systems **scalable, reliable, and performant** through datacenter simulation and digital twins.

## 🔭 Current Focus
* Researching computer systems with the **Massivizing Computer Systems** group, VU Amsterdam.
* Developing **OpenDT**, the first Open-Source Digital Twin for datacenter operation and monitoring *(published with peer-review in [ICPE HotCloudPerf 2026](https://doi.org/10.1145/3777911.3800634))*.
* Simulating the performance and sustainability of **datacenters** with **M3SA** — multi- and meta-model simulation and analysis *(published with peer-review in [Computing Frontiers '26](https://doi.org/10.48550/arXiv.2603.29778))*.
* Leading teaching teams and co-teaching in **Computer Organization** and **Computer Networks** at VU Amsterdam.

## 🤝 Affiliations
[@Large Research](https://atlarge-research.com/) · [SPEC RG Cloud Group](https://research.spec.org/) · [Vrije Universiteit Amsterdam](https://www.vu.nl/en) · [Universiteit van Amsterdam](https://www.uva.nl/en) · [IBM Research Europe](https://research.ibm.com/) *(Dublin & Zurich)*

## 📫 Connect with Me
* **Website:** [radu-nicolae.com](https://radu-nicolae.com)
* **LinkedIn:** [@rnicolae](https://www.linkedin.com/in/rnicolae)
* **Email:** firstName dot lastName 2302 at gmail dotttt com.
